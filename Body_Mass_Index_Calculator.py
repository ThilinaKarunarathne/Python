...
root = Tk()
root.geometry("1350x650+0+3")
root.resizable(5,5)
root.title("BMI (P.M.Thilina Pabasara Karunarathne)")
...
lblTitle=Label(Tops, text= "         Body Mass Index        ",padx=16,pady=16,bd=16, fg="#000000",
               font=('arial',54,'bold'), bg="sky blue", relief='raise', width = 32, height = 1 )
...
lblheight=Label(f1a, text ="Select Weight In Kilogrames",font=('arial',20,'bold'), bd=20) .grid(row=0, column=0)
txtheight = Entry(f1b, textvariable = var2, font=('arial',16,'bold'),bd =16, width=22, justify='center')
txtheight.grid(row = 1, column=0)

lblBMIResult = Label(f1b, padx=16,pady=16,bd=16, fg="#000000",
               font=('arial',30,'bold'),  bg="sky blue", relief='sunk', width = 34, height = 1 )
lblBMIResult.grid(row = 2, column=0)

lblBMITable=Label(f2, font=('arial',20,'bold'), text ="BMI Table").grid(row=0, column=0)
txtlblBMITable = Text(f2, height=13, width=38, bd=16, font=('arial',12,'bold'))
txtlblBMITable.grid(row = 1, column=0)

txtlblBMITable.insert(END, 'Meaning \t\t' + "BMI \n\n")
txtlblBMITable.insert(END, 'Underweight \t\t' + "<19-24,9\n\n")
